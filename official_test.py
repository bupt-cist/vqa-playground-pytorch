...
class VQAEval:
    ...
    def setAccuracy(self, accQA, accQuesType, accAnsType):
        ...
        self.accuracy['perQuestionType'] = {
            ...
            accQuesType}
        # TODO: added perQ_2
        self.accuracy['perQuestionType_2'] = {
            quesType: (sum(accQuesType[quesType]), len(accQuesType[quesType]))
            for quesType in
            accQuesType
            }
...
def clevr_eval(gt_ann, pred_ann):
    gt = file2data(gt_ann)['annotations']
    pred = file2data(pred_ann)
    gt_dict = {}
    for e in gt:
        gt_dict[e['question_id']] = e['multiple_choice_answer']

    assert len(gt) == len(pred), "%s, %s" % (len(gt), len(pred))
    total = 0
    right = 0
    for e in pred:
        qid = e['question_id']
    ...
    print("total: {}, right: {}, accuracy: {:04.2f}%".format(total, right, acc))
    return acc


def test_local(data_dir, result_file, version=2):
    print('<offical_test.py>: Start local_test...')
    ...
    if version == 2:
        ...
        from tdiuc_evaluate import tdiuc_eval
        gt_ann = os.path.join(data_dir, 'tdiuc_mscoco_val2014_annotations.json')
        pred_ann = result_file
        answerkey = os.path.join(data_dir, 'tdiuc_sample_answerkey.csv')
        output_dict = tdiuc_eval(gt_ann=gt_ann, pred_ann=pred_ann, answerkey=answerkey)
        output_filename = os.path.join(os.path.dirname(result_file), 'acc.json')
        with open(output_filename, 'w') as f:
            json.dump(output_dict, f)
        return output_dict['Overall']
    elif version == 'clevr':
        gt_ann = os.path.join(data_dir, 'clevr_mscoco_val2014_annotations.json')
        pred_ann = result_file
        ans = clevr_eval(gt_ann, pred_ann)
    ...
    annFile = os.path.join(data_dir, '%smscoco_val2014_annotations.json' % version_flag)
    quesFile = os.path.join(data_dir, '%sOpenEnded_mscoco_val2014_questions.json' % version_flag)
    ...
    print('<offical_test.py>: Saved results to %s. Overall acc is %s.' % (output_filename, vqaEval.accuracy['overall']))
    return vqaEval.accuracy


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--data_dir', type=str, default='/root/data/VQA/download')
    parser.add_argument('--result_file', type=str,
                        default='/root/Downloads/epoch84_val2015_resfile.json')

    args = parser.parse_args()

    for i in range(1, 10):
        result_file = "/root/data/VQA/logs/EoRG4QGCatTDAll_VAL/epoch_%s/vqa_OpenEnded_mscoco_val2015_EoRG4QGCatTDAll_VAL%03d_results.json" % (i, i)
        out_file = "/root/data/VQA/logs/EoRG4QGCatTDAll_VAL/epoch_%s/acc.json" % (i)
        data2file(test_local(args.data_dir, result_file, version='tdiuc'), out_file)
